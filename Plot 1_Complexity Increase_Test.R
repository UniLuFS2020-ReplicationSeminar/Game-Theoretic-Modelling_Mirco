...
function_break_even <- function (time_cost_share,time_cost_know,y_nonShare,correction,plotmainlabel)  {
  ...
  for (ii in 1:steps) {  
    ...
    time_collect <-  t_paper_var_test  + time_cost_share + t_data_var_test  # This line includes my
    #new variable and my randomly generated datset
    time_no_collect <-  t_paper_var_test  + time_cost_share + time_cost_know + t_search
    ...
    mean(effect_rate)
    mean(effect_rate_full)
    #Defining the cost benefits per researcher
    
    Rate_List$benefits<- round(t_data_var_test*(Rate_List$use_opp/Rate_List$pap_research),3)     # per researcher saved time by reused datasets
    ...
    Rate_List$time<-round(( t_paper_var_test+t_data_var_test-Rate_List$benefits+Rate_List$cost+Rate_List$reusecost)*(pub_research/(t_paper_var+t_data_var)),3)        # ta+td and costs and benefits. Normalize for speed researcher (Tr to t_a+t_d).
